...
const createCopyTitleAndHandle = (title: string, handle: string) => {
	...
	const newHandle = `${handle.replace(/-copy(-\d{1,}){3}(-am|pm])*/g, '')}-copy-${timestamp.replace(/:/g, '-').toLowerCase()}`
...
const useCopyProduct = () => {
	...
	const handleCopyProduct = (product: Product) => {
		const { variants, options, type, tags, images, collection_id, collection, sales_channels, title, handle, ...rest } = omit(
			product,
			['id', 'created_at', 'updated_at', 'deleted_at', 'external_id', 'profile_id', 'profile', 'type_id', 'status']
		)
