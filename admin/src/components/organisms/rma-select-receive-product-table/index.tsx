...
const RMASelectReturnProductTable: React.FC<RMASelectProductTableProps> = ({ order, allItems, toReturn, setToReturn }) => {
	const handleQuantity = (change, item) => {
		if ((item.quantity === toReturn[item.id].quantity && change > 0) || (toReturn[item.id].quantity === 1 && change < 0)) {
			return
		}
