		...
		case 'orders':
			if (query && query.id) {
				...
				value.refund = (payload) => {
					return subcomponent.refund(query.id, payload).then(({ data }) => setResult(data))
				}

				value.updateClaim = (claimId, payload) => {
					return subcomponent.updateClaim(query.id, claimId, payload).then(({ data }) => setResult(data))
				}

				value.createShipment = (payload) => {
					return subcomponent.createShipment(query.id, payload).then(({ data }) => setResult(data))
				}

				value.processSwapPayment = (swapId) => {
					return subcomponent.processSwapPayment(query.id, swapId).then(({ data }) => setResult(data))
				}
				...
				value.createSwap = (payload) => {
					return subcomponent.createSwap(query.id, payload).then(({ data }) => setResult(data))
				}
				...
				value.createClaim = (payload) => {
					return subcomponent.createClaim(query.id, payload).then(({ data }) => setResult(data))
				}

				value.cancelClaim = (claimId) => {
					return subcomponent.cancelClaim(query.id, claimId).then(({ data }) => setResult(data))
				}

				value.fulfillSwap = (swapId, payload) => {
					return subcomponent.fulfillSwap(query.id, swapId, payload).then(({ data }) => setResult(data))
				}
				...
				value.fulfillClaim = (claimId, payload) => {
					return subcomponent.fulfillClaim(query.id, claimId, payload).then(({ data }) => setResult(data))
				}
				...
				value.createFulfillment = (payload) => {
					return subcomponent.createFulfillment(query.id, payload).then(({ data }) => setResult(data))
				}

				value.cancelFulfillment = (fulfillmentId) => {
					return subcomponent.cancelFulfillment(query.id, fulfillmentId).then(({ data }) => setResult(data))
				}

				value.requestReturn = (payload) => {
					return subcomponent.requestReturn(query.id, payload).then(({ data }) => setResult(data))
				}

				value.receiveReturn = (returnId, payload) => {
					return subcomponent.receiveReturn(returnId, payload).then(({ data }) => setResult(data))
				}

				value.cancelReturn = (returnId) => {
					return subcomponent.cancelReturn(returnId).then(({ data }) => setResult(data))
				}

				value.cancel = () => {
					return subcomponent.cancel(query.id).then(({ data }) => setResult(data))
				}
			}
			break
		case 'store':
		...
		case 'products':
			if (query && query.id) {
				const variantMethods = {
					create: (variant) => {
						return subcomponent.variants.create(query.id, variant).then(({ data }) => {
							setResult(data)
						})
					},
					retrieve: (variantId) => {
						return subcomponent.variants.retrieve(query.id, variantId).then(({ data }) => {
							setResult(data)
						})
					},
					update: (variantId, update) => {
						return subcomponent.variants.update(query.id, variantId, update).then(({ data }) => {
							setResult(data)
						})
					},
					delete: (variantId) => {
						return subcomponent.variants.delete(query.id, variantId).then(({ data }) => {
							setResult(data)
						})
					},
					list: () => {
				...
				const optionMethods = {
					create: (option) => {
						return subcomponent.options.create(query.id, option).then(({ data }) => {
							setResult(data)
						})
					},
					update: (optionId, update) => {
						return subcomponent.options.update(query.id, optionId, update).then(({ data }) => {
							setResult(data)
						})
					},
					delete: (optionId) => {
						return subcomponent.options.delete(query.id, optionId).then(({ data }) => {
							setResult(data)
						})
					},
				}
