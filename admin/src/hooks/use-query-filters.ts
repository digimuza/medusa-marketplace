...
const useQueryFilters = (defaultFilters: Partial<FilterState>) => {
	...
	const setFilters = (path: string, value: any) => dispatch({ type: FilterActionType.SET_FILTERS, path, payload: value })

	const setQuery = (queryString: string | undefined) => dispatch({ type: FilterActionType.SET_QUERY, payload: queryString })
